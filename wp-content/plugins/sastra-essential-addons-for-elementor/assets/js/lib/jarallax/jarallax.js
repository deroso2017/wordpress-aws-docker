	...
	class Jarallax {
		...
		onScroll(force) {
			...
			if (contH < wndH) {
				/**
                 * Option 1 (not used)
                 * Effect is applied only when the section starts entering or leaving the viewport.
                 * While the section is centered in the viewport, the effect is neutral/normal.
                 */
				// visiblePercent = 1 - (afterTop || beforeBottom) / contH;

				/**
                 * Option 2 (currently used)
                 * Effect is applied continuously from the moment the section enters the viewport until it leaves.
                 * Creates a smooth, single continuous effect across the whole scroll.
                 */
				visiblePercent = fromViewportCenter * 0.5 + 0.5;
			...
			if (self.options.type === 'opacity' || self.options.type === 'scale-opacity' || self.options.type === 'scroll-opacity') {
				...
				// styles.opacity = visiblePercent;   // opacity decreases from 1 → 0
				styles.opacity = 1 - visiblePercent;  // opacity increases from 0 → 1
			}
