...
trait Template_Query {
    ...
    private function process_directory_name()
    {
        ...
        $widget_name = str_replace('tmpcoder-', '', $this->current_widget_name);
        $widget_name = str_replace('-', ' ', $widget_name);
        $widget_name = ucwords($widget_name);
        $widget_name = str_replace(' ', '-', $widget_name);
        return 'classic-widget-templates';
    }
    ...
    private function get_template_dir() {
        return \sprintf(
            '%sinc/woocommerce/%s',
            TMPCODER_PLUGIN_DIR, $this->process_directory_name()
        );
    }
    ...
    {
        if (!apply_filters('tmpcoder/is_plugin_active', 'spexo-elementor-addons/essential_adons_elementor.php')) {
            return false;
        }
        return \sprintf(
