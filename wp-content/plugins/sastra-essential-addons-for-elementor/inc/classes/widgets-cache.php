...
class Widgets_Cache {
    ...
    const OPTION_KEY = 'spexoaddons_elements_cache';

    const META_KEY = '_spexoaddons_elements_cache';
    ...
    protected function get_global_widget_type( $template_id ) {
        $template_data = tmpcoder_elementor()->templates_manager->get_template_data( [
            'source' => 'local',
        ...
        $original_widget_type = tmpcoder_elementor()->widgets_manager->get_widget_types( $template_data['content'][0]['widgetType'] );
    ...
    public function get() {
        ...
        return array_map( function( $widget_key ) {
            return str_replace( 'tmpcoder-', '', $widget_key );
        }, array_keys( $cache ) );
    }
    ...
    public function get_elementor_data() {
        ...
        if ( is_null( $this->elementor_data ) ) {
            $document = tmpcoder_elementor()->documents->get( $this->get_post_id() );
    ...
    public function save() {
        ...
        $cache = [];
        tmpcoder_elementor()->db->iterate_data( $data, function ( $element ) use ( &$cache ) {
            $type = $this->get_widget_type( $element );

            if ( strpos( $type, 'tmpcoder-' ) !== false ) {
                if ( ! isset( $cache[ $type ] ) ) {
                    $cache[ $type ] = 0;
                }
                $cache[ $type ] ++;
