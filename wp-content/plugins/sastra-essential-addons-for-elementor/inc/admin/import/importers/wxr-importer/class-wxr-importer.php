 ...
 * WXR Importer
 */
if ( ! class_exists( 'TMPCODER_Wxr_Importer' ) && class_exists( 'WP_Importer' ) ) :
	/**
	 * WXR Importer
	 */
	class TMPCODER_Wxr_Importer extends WP_Importer {
		/**
		 ...
		 * @var array
		 */
		protected $secondary_images = array();
		/**
		 * Logger instance.
		 *
		 * @var TMPCODER_Importer_Logger
		 */
		protected $logger;
		/**
		 * Constructor
		...
		protected function get_reader( $file ) {
			...
			if ( ! $status ) {
				return new WP_Error( 'tmpcoder_importer.cannot_parse', __( 'Could not open the file for parsing', 'sastra-essential-addons-for-elementor' ) );
			}
			return $reader;
		}
		/**
						...
						if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
							$this->logger->warning(
								sprintf(
									...
									__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'sastra-essential-addons-for-elementor' ),
									$this->version,
		 ...
		 * @param string $file Path to the WXR file for importing.
		 */
		public function import( $file, $tmpcoder_cpt_data='' ) {
			add_filter( 'import_post_meta_key', array( $this, 'is_valid_meta_key' ) );
			add_filter( 'http_request_timeout', array( &$this, 'bump_request_timeout' ) );
			if (!empty($tmpcoder_cpt_data)) {
				$this->tmpcoder_register_tmp_post_types($tmpcoder_cpt_data);
			}
			...
			while ( $reader->read() ) {
				...
				switch ( $reader->name ) {
					...
					case 'item':
						...
						/* Skip duplicate upload items when retry call [start] */
						$last_import_id = get_option('tmpcoder_already_exists_post'); 
						$tmpcoder_is_call_retry = get_option('tmpcoder_is_call_retry');
						if ($tmpcoder_is_call_retry == 'yes' && $last_import_id != '' && ($last_import_id == $parsed['data']['post_id'] || $last_import_id != $parsed['data']['post_id']) ) {
							if ($last_import_id == $parsed['data']['post_id']) {
								$tmpcoder_is_call_retry = delete_option('tmpcoder_is_call_retry');
							}
							continue 2;				
						}
						else
		...
		public function tmpcoder_register_tmp_post_types($custom_post_type){
			if (!empty($custom_post_type)) {
				$custom_post_type = json_decode(stripslashes($custom_post_type),true);
				if (!empty($custom_post_type)) {
					foreach ($custom_post_type as $post_type => $value) {
						// Register custom post type
						register_post_type($value['post'], array(
							'public' => true,
							'has_archive' => true,
						...
						if (isset($value['tax']) && !empty($value['tax'])) {
							foreach ($value['tax'] as $key => $tax_val) {
								register_taxonomy($tax_val, $value['post'], array(
									'public' => true,
									'show_in_nav_menus' => true,
									'hierarchical' => true
								));
		...
		protected function import_start( $file ) {
			if ( ! is_file( $file ) ) {
				return new WP_Error( 'tmpcoder_importer.file_missing', __( 'The file does not exist, please try again.', 'sastra-essential-addons-for-elementor' ) );
			}
			 ...
			 */
			do_action( 'tmpcoder_import_start' );
		}
		/**
		...
		public function set_user_mapping( $mapping ) {
			foreach ( $mapping as $map ) {
				if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
					$this->logger->warning( __( 'Invalid author mapping', 'sastra-essential-addons-for-elementor' ) );
				...
				switch ( $child->tagName ) {
					...
					case 'wp:status':
						...
						if ( 'auto-draft' === $data['post_status'] ) {
							// Bail now.
							return new WP_Error(
								'tmpcoder_importer.post.cannot_import_draft',
								__( 'Cannot import auto-draft posts', 'sastra-essential-addons-for-elementor' ),
								$data
							);
						}
						break;
		...
		protected function process_post( $data, $meta, $comments, $terms ) {
			 ...
			 * @param array $terms Terms on the post.
			 */
			$data = apply_filters( 'tmpcoder_importer.pre_process.post', $data, $meta, $comments, $terms ); 
			if ( empty( $data ) ) {
				return false;
			}
			...
			if ( ! $post_type_object ) {
				$this->logger->warning(
					sprintf(
						...
						__( 'Failed to import "%1$s": Invalid post type %2$s', 'sastra-essential-addons-for-elementor' ),
						$data['post_title'],
						$data['post_type']
					)
				);
			...
			if ( $post_exists ) {
				$message = sprintf(
					...
					__( '%1$s "%2$s" already exists.', 'sastra-essential-addons-for-elementor' ),
				 ...
				 */
				do_action( 'tmpcoder_importer.process_already_imported.post', $data ); 
				...
				if ( ! $this->options['fetch_attachments'] ) {
					$this->logger->notice(
						sprintf(
							...
							__( 'Skipping attachment "%s", fetching attachments disabled', 'sastra-essential-addons-for-elementor' ),
							$data['post_title']
						)
					);
					/**
					 ...
					 */
					do_action( 'tmpcoder_importer.process_skipped.post', $data, $meta ); 
					return false;
				}
			...
			if ( is_wp_error( $post_id ) ) {
				 ...
				 */
				do_action( 'tmpcoder_importer.process_failed.post', $post_id, $data, $meta, $comments, $terms ); 
				return false;
			}
			...
			$this->logger->info(
				sprintf(
					/* translators: %1$s is the post title, %2$s is post type. */
					__( 'Imported "%1$s" (%2$s)', 'sastra-essential-addons-for-elementor' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
			$this->logger->debug(
				sprintf(
					...
					__( 'Post %1$d remapped to %2$d', 'sastra-essential-addons-for-elementor' ),
					$original_id,
					$post_id
				)
			);
			...
			add_post_meta($post_id, 'tmpcoder_old_id', $original_id);
			$this->process_comments( $comments, $post_id, $data );
			$this->process_post_meta( $meta, $post_id, $data );
			if ( 'nav_menu_item' === $data['post_type'] ) {
				$this->process_menu_item_meta( $post_id, $data, $meta );
			}
			update_option('tmpcoder_already_exists_post', $original_id);
			/**
			 ...
			 * @param array $terms Raw term data, already processed.
			 */
			do_action( 'tmpcoder_importer.processed.post', $post_id, $data, $meta, $comments, $terms ); 
		}
		/**
		...
		protected function process_attachment( $post, $meta, $remote_url ) {
			...
			if ( ! $info ) {
				return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'sastra-essential-addons-for-elementor' ) );
			}
		...
		protected function process_post_meta( $meta, $post_id, $post ) {
			...
			foreach ( $meta as $meta_item ) {
				 ...
				 */
				$meta_item = apply_filters( 'tmpcoder_importer.pre_process.post_meta', $meta_item, $post_id ); 
				if ( empty( $meta_item ) ) {
					return false;
				}
				...
				if ( $key ) {
					...
					if ($key == 'tmpcoder_target_include_locations') {
						if (isset($value['specific'])) {
							foreach ($value['specific'] as $meta_key => $meta_value) {
								$old_id = explode('-', $meta_value)[1];
								if (!empty($old_id)) {
									if (explode('-', $meta_value[0]) == 'post') {
										$new_id = tmpcoder_get_post_id_by_meta_key_and_meta_value('tmpcoder_old_id', $old_id);

										if (!empty($new_id) && $new_id != $old_id) {
											$new_value = str_replace('post-'.$old_id, 'post-'.$new_id, $meta_value);
										}
										else
										{
									...
									{
										global $wpdb;
										$new_id = $wpdb->get_var( $wpdb->prepare("SELECT term_id FROM $wpdb->termmeta WHERE meta_key = %s AND meta_value = %s ", 'tmpcoder_old_id', $old_id) );
										if (!empty($new_id) && $new_id != $old_id) {
										
											$new_value = str_replace('tax-'.$old_id, 'tax-'.$new_id, $meta_value);
										}
										else
										{
											$new_value = $meta_value;
								...
								$value['specific'][$meta_key] = $new_value;
							}
						}
					}
                    ...
                    if ( 'tmpcoder_secondary_image_id' === $key ) {
						$this->secondary_images[ $post_id ] = (int) $value;
					}
				}
			}
			return true;
		...
		protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
			...
			foreach ( $comments as $key => $comment ) {
				 ...
				 */
				$comment = apply_filters( 'tmpcoder_importer.pre_process.comment', $comment, $post_id ); 
				if ( empty( $comment ) ) {
					return false;
				}
				...
				if ( $post_exists ) {
					...
					if ( $existing ) {
						 ...
						 */
						do_action( 'tmpcoder_importer.process_already_imported.comment', $comment ); 
				 ...
				 */
				do_action( 'tmpcoder_importer.processed.comment', $comment_id, $comment, $meta, $post_id );
				$num_comments++;
			}
			return $num_comments;
		}
		...
		protected function process_author( $data, $meta ) {
			 ...
			 * @param array $meta Meta data.
			 */
			$data = apply_filters( 'tmpcoder_importer.pre_process.user', $data, $meta ); 
			if ( empty( $data ) ) {
				return false;
			}
			...
			if ( is_wp_error( $user_id ) ) {
				$this->logger->error(
					sprintf(
						...
						__( 'Failed to import user "%s"', 'sastra-essential-addons-for-elementor' ),
						$userdata['user_login']
					)
				);
				 ...
				 */
				do_action( 'tmpcoder_importer.process_failed.user', $user_id, $userdata ); 
				return false;
			}
			if ( $original_id ) {
			...
			$this->logger->info(
				sprintf(
					/* translators: %s user login name  */
					__( 'Imported user "%s"', 'sastra-essential-addons-for-elementor' ),
					$userdata['user_login']
				)
			);
			$this->logger->debug(
				sprintf(
					...
					__( 'User %1$d remapped to %2$d', 'sastra-essential-addons-for-elementor' ),
					$original_id,
					$user_id
				)
			);
			 ...
			 * @param array $userdata Raw data imported for the user.
			 */
			do_action( 'tmpcoder_importer.processed.user', $user_id, $userdata ); 
		}
		/**
		 * Process Term
		 *
		...
		protected function process_term( $data, $meta ) {
			 ...
			 * @param array $meta Meta data.
			 */
			$data = apply_filters( 'tmpcoder_importer.pre_process.term', $data, $meta ); 
			if ( empty( $data ) ) {
				return false;
			}
			...
			if ( $existing ) {
				 ...
				 */
				do_action( 'tmpcoder_importer.process_already_imported.term', $data ); 
			...
			if ( is_wp_error( $result ) ) {
				 ...
				 */
				do_action( 'tmpcoder_importer.process_failed.term', $result, $data, $meta ); 
				return false;
			}
			...
			$this->logger->debug(
				sprintf(
					...
					__( 'Term %1$d remapped to %2$d', 'sastra-essential-addons-for-elementor' ),
					$original_id,
					$term_id
				)
			);
            add_term_meta($term_id,'tmpcoder_old_id',$original_id);
			 ...
			 * @param array $data Raw data imported for the term.
			 */
			do_action( 'tmpcoder_importer.processed.term', $term_id, $data ); 
		}
        /**
		...
		protected function fetch_remote_file( $url, $post ) {
			...
			if ( 200 !== $code ) {
				...
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Remote server returned %1$d %2$s for %3$s', 'sastra-essential-addons-for-elementor' ),
						$code,
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				$message = sprintf( __( 'Remote file is too large, limit is %s', 'sastra-essential-addons-for-elementor' ), size_format( $max_size ) );
			...
			foreach ( $todo as $post_id => $_ ) {
				$this->logger->debug(
					sprintf(
						...
						__( 'Running post-processing for post %d', 'sastra-essential-addons-for-elementor' ),
						$post_id
					)
				);
				$data = array();
					...
					if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'sastra-essential-addons-for-elementor' ),
						...
						$this->logger->debug(
							sprintf(
								...
								__( 'Post %1$d was imported with parent %2$d, but could not be found', 'sastra-essential-addons-for-elementor' ),
								$post_id,
								$parent_id
							)
						);
					...
					if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the author for "%1$s" (post #%2$d)', 'sastra-essential-addons-for-elementor' ),
								get_the_title( $post_id ),
								$post_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Post %1$d was imported with author "%2$s", but could not be found', 'sastra-essential-addons-for-elementor' ),
								$post_id,
								$author_slug
							)
						);
                ...
                if ( get_post_type( $post_id ) == 'attachment' && isset($data['post_parent']) && $data['post_parent'] != "" && $post_id != "" ){
                    // features images set
                    // update_post_meta( $data['post_parent'], '_thumbnail_id', $post_id );
                }
				...
				if ( empty( $data ) ) {
					$this->logger->debug(
						sprintf(
							/* translators: %d is post id. */
							__( 'Post %d was marked for post-processing, but none was required.', 'sastra-essential-addons-for-elementor' ),
							$post_id
						)
					);
					continue;
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning(
						sprintf(
							/* translators: %1$s is the post title, %2$s is post id. */
							__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'sastra-essential-addons-for-elementor' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
		...
		protected function post_process_menu_item( $post_id ) {
			...
			if ( ! empty( $menu_object ) ) {
				...
				$this->logger->warning(
					sprintf(
						/* translators: %1$s is the post title, %2$s is post id. */
						__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'sastra-essential-addons-for-elementor' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
				$this->logger->debug(
					sprintf(
						...
						__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'sastra-essential-addons-for-elementor' ),
						$post_id,
						$menu_object_id,
						$menu_item_type
					)
			...
			foreach ( $todo as $comment_id => $_ ) {
					...
					if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								...
								__( 'Could not find the comment parent for comment #%d', 'sastra-essential-addons-for-elementor' ),
								$comment_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'sastra-essential-addons-for-elementor' ),
								$comment_id,
								$parent_id
							)
						);
				...
				if ( ! empty( $author_id ) ) {
					...
					if ( isset( $this->mapping['user'][ $author_id ] ) ) {
						...
						$this->logger->warning(
							sprintf(
								/* translators: %d is comment id. */
								__( 'Could not find the author for comment #%d', 'sastra-essential-addons-for-elementor' ),
								$comment_id
							)
						);
						$this->logger->debug(
							sprintf(
								...
								__( 'Comment %1$d was imported with author %2$d, but could not be found', 'sastra-essential-addons-for-elementor' ),
								$comment_id,
								$author_id
							)
						);
				...
				if ( empty( $result ) ) {
					$this->logger->warning(
						sprintf(
							/* translators: %d is comment id. */
							__( 'Could not update comment #%d with mapped data', 'sastra-essential-addons-for-elementor' ),
							$comment_id
						)
					);
					continue;
        ...
        private function remap_featured_images() {
            ...
            // Cycle through posts that have a secondary image.
            foreach ( $this->secondary_images as $post_id => $value ) {
                if ( isset( $this->processed_posts[ $value ] ) ) {
                    $new_id = $this->processed_posts[ $value ];
                    // Only update if there's a difference.
                    if ( $new_id !== $value ) {
                        update_post_meta( $post_id, 'tmpcoder_secondary_image_id', $new_id );
                    }
                }
            }
        }
